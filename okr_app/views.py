...
from .permissions import IsApplicationAdminUser, IsOKROwner, IsOKROwnerMentor
...
class OKRFileViewset(viewsets.GenericViewSet,
    ...
    def get_queryset(self):
        ...
        if okr and (self.check_okr_permission(okr) or
           IsOKROwnerMentor().has_object_permission(self.request, None, okr)):
...
class OKRNotifyView(APIView):
    ...
    def email(self, okr):
        if hasattr(self.request.user, 'mentorship'):
            for mentor in self.request.user.mentorship.mentor.all():
                if mentor.email:
